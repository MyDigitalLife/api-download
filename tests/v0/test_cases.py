...
def test_index_returns_stream_for_existing_uuid(app):
...
def test_index_returns_stream_for_random_uuid(app):
    with app.app_context():
        res = app.test_client().get(url_for("v0.cases.index", uuid=uuid4()))
        assert res.status_code == 200
        assert res.is_streamed


def test_index_fails_without_args(app):
    with app.app_context():
        res = app.test_client().get(url_for("v0.cases.index"))
